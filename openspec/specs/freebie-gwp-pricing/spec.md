# freebie-gwp-pricing Specification

## Purpose

Verifies gift-with-purchase (GWP) pairing at checkout time and zeroes a freebie cart line's price server-side, via a Cart Transform Function, whenever a valid pairing with its qualifying main product is still present in the cart. Does not decide GWP eligibility (that's produced by separate client-side logic) — only re-verifies the pairing still holds and enforces the price change in a way the customer cannot tamper with.
